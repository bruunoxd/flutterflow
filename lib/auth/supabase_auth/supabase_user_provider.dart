...
class FinancasSupabaseUser extends BaseAuthUser {
  FinancasSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> financasSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = FinancasSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
